...
export function ClaimPacket() {
  ...
  const { metadata, refetch, isLoading, isError } = useRedPacket({
    contractAddress,
  })
  ...
  if (isLoading) {
    return <Loading message="Available packets checking..." />
  }

  if (isPending) {
    return <Loading message="Confirming packet details..." />
  }
